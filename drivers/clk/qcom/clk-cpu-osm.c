...
#ifdef CONFIG_VOLTAGE_VIEWER
static int read_opp(struct clk_osm *c, struct device **device_list, int count)
{
	unsigned long rate = 0;
	unsigned int uv;
	long rc;
	...
	while (1) {
		...
		for (i = 0; i < count; i++) {
			rc = dev_pm_opp_add(device_list[i], rate, uv);
			if (rc) {
				pr_warn("failed to read OPP for %lu\n", rate);
				return rc;
			}
		}

		for (i = 0; i < count; i++) {
			if (strcmp (dev_name(device_list[i]), "cpu0") == 0) {
				strcpy (pwrcl_clk_vdd.dev_name[j * count + i - 1], dev_name(device_list[i]));
				pwrcl_clk_vdd.rate[j * count + i - 1] = rate;
				pwrcl_clk_vdd.uv[j * count + i - 1] = uv;
				pwrcl_clk_vdd.size = (j + 1) * count -1;
			}
			else if (strcmp (dev_name(device_list[i]), "cpu4") == 0) {
				strcpy (perfcl_clk_vdd.dev_name[j * count + i - 1], dev_name(device_list[i]));
				perfcl_clk_vdd.rate[j * count + i - 1] = rate;
				perfcl_clk_vdd.uv[j * count + i - 1] = uv;
				perfcl_clk_vdd.size = (j + 1) * count -1;
			}
			else {
				strcpy (dev_clk_vdd.dev_name[j * count + i - 1], dev_name(device_list[i]));
				dev_clk_vdd.rate[j * count + i - 1] = rate;
				dev_clk_vdd.uv[j * count + i - 1] = uv;
				dev_clk_vdd.size = (j + 1) * count -1;
			}
		}
...
#ifdef CONFIG_VOLTAGE_VIEWER
	if (read_opp(&l3_clk, device_list, count))
		pr_err("Failed to read OPP levels for %s\n", phandle_name);
#endif
	kfree(device_list);
}
